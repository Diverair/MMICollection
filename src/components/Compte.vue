    ...
    <div class="app fond">
            ...
            <div id="connexion" class="modal-dialog fond connexion" role="document">
                <div class="cadre-connexion modal-content">
                    <div class="modal-header">
                        <form @submit.prevent="BanniereUpload" enctype="multipart/form-data">
                            ...
                            <br>
                            <label for="mdp-connexion">Votre mot de passe</label>
                            <input type="password" id="mdp-connexion" placeholder="Mot-de-passe" v-model="Membre.mdp"><br>
        ...
        <section class="profil">
            <div class="image-avatar">
                <div class="banniere">
                    <img :src="banniereData" alt="Bannière par défaut">
                ...
                <div class="fond-avatar">
                    <img :src="avatarData" class="avatar" alt="Bannière par défaut">
                </div>
            ...
            <div class="profil-info">       
                <div class="profil-desc">
                    ...
                    <br>
                    <i v-if="Membre.acf.role != 'subscriber' && Membre.acf.specialite == null || Membre.acf.specialite == 0 &&  Membre.acf.role != 'subscriber'">Aucune spécialité pour le moment (S4)</i>
                    <p v-if="Membre.acf.specialite[0] != null ">{{Membre.acf.specialite[0].post_title}}</p>
        ...
        <div class="scroll-snap">
            <section class="section-projet Projets-similaires" id="projets-realises">
                <div class="ligne-titre">
                    <h3>Projet(s) réalisé(s)</h3>
                    <hr>
                </div>
                <ul class="carrousel">
                    <li class="choix">
                        <div v-for="roleprojet in listeRoleProjet" :key="roleprojet.id">
                            <div v-if="roleprojet.utilisateur[0].ID == Membre.id">
                                <router-link :to="{name: 'Projet', params: {id : projet.id}}"  v-for="projet in listeProjets" :key="projet.id">
                                    <div v-if="roleprojet.projet[0].ID == projet.id">
                                        <div>
                                            <img :src="projet.image.url" class="visuel" alt="Images projet">
                                        </div>
                                        <div>
                                            <img :src="projet.logo.url" class="logo-projet" alt="logo projet">
                                            <h6>{{projet.nom}}</h6>
                                            <br>               
                                            ...
                                            <div v-for="role in listeRoles" :key="role.id">
                                                <div v-if="roleprojet.role.length == 1">
                                                    <i v-if="role.id == roleprojet.role[0].ID">{{role.nom}}</i>  
                                                </div>  
                                                <div v-if="roleprojet.role.length == 2">
                                                    <i v-if="role.id == roleprojet.role[0].ID">{{role.nom}}</i>  
                                                    <i v-if="role.id == roleprojet.role[1].ID">{{role.nom}}</i>  
                                                </div>  
                                            </div>
            ...
            <!--<section class="section-projet" id="Acteurs" v-for="roleprojet in listeRoleProjet" :key="roleprojet.id">
                <div v-if="roleprojet.acf.utilisateur[0].ID == Membre.id">
                    <div class="ligne-titre">
                        <h3>Co-équipier</h3>
                        <hr>
                    </div>
...
export default {
    ...
    created(){
        ...
        .then(response=>{
            ...
            if(this.Membre.acf.banniere == null || this.Membre.acf.banniere == false){
                this.banniereData = this.imageDefautBanniere; 
            }else{
                this.banniereData = this.Membre.acf.banniere.url;
            }

             if(this.Membre.acf.logo == null || this.Membre.acf.logo == false){
        ...
        axios.get(param.host+"role/v2/listeRoles")
        .then(response=>{
    ...
    methods:{
        ...
        BanniereUpload: function(){
            ...
            this.message = param.message.wait
            ...
            .then(function (response){
                ...
                .then(function (response){
                    ...
                    if(idBanniere != undefined){
                        ...
                        .then(function(response){
                            console.log("Retour modification Membre", response);
                            this.$router.push('/compte/'+this.Membre.id);
                            $("#BanModal").modal('hide');
                            location.relaod();
                        }.bind(this))
                        .catch(error => 
                            this.message = param.message.errCreaImage 
                        )
                    }
                }
                .bind(this))
...
.choix>div>div>a>div>div:last-of-type{
    padding: 15px;
    width: calc(100% - 30px);
    height:max-content;
}

.choix>div>div>a>div>div>img.visuel{
    max-height: 110%;
    min-height: 100%;
    min-width: 100%;
    box-sizing: inherit;
...
.choix>div>div>a:hover>div>div>img.visuel{
    transform-origin: center;
    transform: scale(1.15) translateX(-45%) translateY(-45%);
}

.choix>div>div>a>div>div>h6{
    width: 100%;
    margin: 0;
}
</style>
